...
from ntro_srm.evaluation import evaluate_geotiffs
...
def test_ssim_ignores_values_outside_valid_mask() -> None:
    reference = np.full((1, 15, 15), 0.2, dtype=np.float64)
    prediction = reference.copy()
    prediction[:, 5:10, 5:10] = 0.4
    valid = np.ones((15, 15), dtype=bool)
    valid[7, 7] = False

    baseline = evaluate_arrays(prediction, reference, valid_mask=valid)
    prediction[:, 7, 7] = 1000.0
    reference[:, 7, 7] = -1000.0
    with_invalid_outliers = evaluate_arrays(prediction, reference, valid_mask=valid)

    assert np.isclose(
        baseline["aggregate"]["mean_ssim"],
        with_invalid_outliers["aggregate"]["mean_ssim"],
    )
...
def test_geotiff_source_consistency_and_reference_report(tmp_path: Path) -> None:
    ...
    _write_raster(
        source_path,
        source * 10000.0,
        Affine(10, 0, 500000, 0, -10, 3000000),
        S2_10BAND_NAMES,
    )
...
def test_geotiff_reference_uses_explicit_non_sentinel_scale(tmp_path: Path) -> None:
    rng = np.random.default_rng(21)
    prediction = rng.uniform(0.05, 0.95, size=(3, 12, 12)).astype(np.float32)
    transform = Affine(2.5, 0, 500000, 0, -2.5, 3000000)
    prediction_path = tmp_path / "prediction.tif"
    reference_path = tmp_path / "reference_8bit.tif"

    _write_raster(prediction_path, prediction, transform)
    _write_raster(reference_path, prediction * 255.0, transform)

    report = evaluate_geotiffs(
        prediction_path,
        reference_path=reference_path,
        reference_scale=255.0,
    )
