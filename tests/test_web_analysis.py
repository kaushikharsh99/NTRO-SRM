...
from ntro_srm.web.schemas import InferenceRequest
...
class TestReportDownloads:
    ...
    def test_unknown_file_type_rejected(self, client: TestClient, completed_job: dict):
        ...
        assert resp.status_code == 400
# Expensive validation remains available but must not silently multiply the
# latency of the default CPU/demo path.
def test_expensive_analysis_is_opt_in_by_default():
    request = InferenceRequest(is_demo=True)
    assert request.run_analysis is True
    assert request.run_wald_validation is False
    assert request.uncertainty_members == 0
