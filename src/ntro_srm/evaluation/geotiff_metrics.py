...
def _ssim_per_band(
    ...
    valid = torch.from_numpy(valid_mask).reshape(1, 1, height, width)
    x = torch.from_numpy(prediction).unsqueeze(1)
    y = torch.from_numpy(reference).unsqueeze(1)
    x = torch.where(valid, x, 0.0)
    y = torch.where(valid, y, 0.0)
    window = _gaussian_window(window_size)
    padding = window_size // 2

    local_weight = F.conv2d(valid.to(torch.float64), window, padding=padding).clamp_min(1e-15)
    mu_x = F.conv2d(x, window, padding=padding) / local_weight
    mu_y = F.conv2d(y, window, padding=padding) / local_weight
    mu_x_sq = mu_x.square()
    mu_y_sq = mu_y.square()
    mu_xy = mu_x * mu_y
    sigma_x_sq = F.conv2d(x.square(), window, padding=padding) / local_weight - mu_x_sq
    sigma_y_sq = F.conv2d(y.square(), window, padding=padding) / local_weight - mu_y_sq
    sigma_xy = F.conv2d(x * y, window, padding=padding) / local_weight - mu_xy
