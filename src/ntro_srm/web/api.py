...
def get_health(request: Request) -> HealthResponse:
    ...
    return HealthResponse(
        ...
        accelerator_available=info.accelerator_available,
        accelerator_active=info.accelerator_active,
...
async def upload_geotiff_patch(
    ...
    uncertainty_members: Optional[int] = Query(
        default=0, ge=0, le=8, description="Test-time-augmentation ensemble size (0 = novelty only)"
    ),
) -> dict:
